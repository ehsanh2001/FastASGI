...
from fastasgi.testing.response import TestResponse
...
class TestFastASGI:
    """Test the FastASGI application with routing using TestClient."""
...
class TestRouteMatching:
    ...
    def test_multiple_http_methods(self):
        ...
        test_request = TestRequest().set_query_params(q="fastasgi", limit="20")
        response = client.get("/search", test_request)

        assert response.status_code == 200
        data = response.json()
        assert data["query"] == "fastasgi"
    ...
    def test_route_ordering_and_conflicts(self):
        ...
        # Less specific route - note: FastASGI requires path parameters to be in handler signature
