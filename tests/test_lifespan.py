...
class TestLifespanEvents:
    ...
    def test_shutdown_event_decorator(self):
        ...
        # Verify handler was registered (plus the default cleanup handler)
    ...
    def test_add_event_handler_shutdown(self):
        ...
        # Verify handler was registered (plus the default cleanup handler)
        assert len(app._shutdown_handlers) == 2
        assert handler in app._shutdown_handlers
    ...
    def test_multiple_shutdown_handlers(self):
        ...
        # Verify all handlers were registered (plus the default cleanup handler)
        assert len(app._shutdown_handlers) == 3
        assert shutdown_handler in app._shutdown_handlers
    ...
    def test_empty_handlers_dont_error(self):
        ...
        # Verify no user handlers are registered initially (plus built-in handlers)
        assert len(app._startup_handlers) == 1  # _build_middleware_chain
        assert len(app._shutdown_handlers) == 1  # _cleanup_all_requests
